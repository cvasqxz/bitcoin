...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ...
        ChainTypeToString(ChainType::TESTNET2),
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Chaucha
    //   new: C:\Users\Username\AppData\Local\Chaucha
    // macOS: ~/Library/Application Support/Chaucha
    // Unix-like: ~/.chaucha
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Chaucha";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Chaucha";
#else
    fs::path pathRet;
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fTestNet2 > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -testnet2 and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fTestNet2) return ChainType::TESTNET2;
